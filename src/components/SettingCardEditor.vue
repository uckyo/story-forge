    ...
    <el-form label-width="80px">
      ...
      <el-form-item label="类别">
        <el-input
          ...
          placeholder="设定类别，如：魔法、科技、历史、政治等"
        ></el-input>
      </el-form-item>
      <el-form-item label="影响范围">
        <el-input
          v-model="formData.influence"
          placeholder="该设定影响的范围"
        ></el-input>
      </el-form-item>
      ...
      >
        <ul class="p-1">
          ...
          >
            ...
            <el-icon v-else-if="card.type === 'item'"><Goods /></el-icon>
            <el-icon v-else-if="card.type === 'event'"><Document /></el-icon>
...
const filteredMentionCards = computed(() => {
  if (!mentionKeyword.value) {
    return bookStore.currentBookCards
      .filter((card) => card.id !== formData.id)
      .slice(0, 10);
  }
  return bookStore.currentBookCards
    .filter(
...
const handleContentInput = (event) => {
  ...
  if (
    lastAtIndex !== -1 &&
    /^[^\s]*$/.test(textBeforeCursor.substring(lastAtIndex + 1))
  ) {
...
const selectMention = (card) => {
  ...
  const cursorPosition =
    content.lastIndexOf("@" + mentionKeyword.value) +
    mentionKeyword.value.length +
    1;
  const newContent =
    content.substring(0, content.lastIndexOf("@")) + "@" + card.name + " ";
  ...
  nextTick(() => {
    const input = document.querySelector(
      'textarea[placeholder="详细描述（支持@提及其他卡片）"]'
    );
    if (input) {
      input.focus();
